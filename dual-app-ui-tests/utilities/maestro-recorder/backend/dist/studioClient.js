...
exports.hashHierarchy = hashHierarchy;
exports.invalidateStudioAvailability = invalidateStudioAvailability;
exports.tryStudioSnapshot = tryStudioSnapshot;
exports.studioEndpoint = studioEndpoint;
...
const agent = new http_1.default.Agent({ keepAlive: true, maxSockets: 4 });
let studioAvailable = null; // tri-state cache
...
function hashHierarchy(elements) {
...
function normalizeStudioNode(node, out, index = { i: 0 }) {
    ...
    const text = (attrs['text'] ??
        ...
        attrs['content-desc'] ??
        '').toString();
    const bounds = (attrs['bounds'] ?? attrs['frame'] ?? '').toString() || '0,0,0,0';
    const clickable = attrs['clickable'] === 'true' ||
        attrs['clickable'] === true ||
        attrs['enabled'] === 'true';
    const focused = attrs['focused'] === 'true' || attrs['focused'] === true;
    ...
    for (const child of children)
        normalizeStudioNode(child, out, index);
}
async function isStudioReachable() {
    const now = Date.now();
    if (studioAvailable !== null &&
...
function invalidateStudioAvailability() {
 ...
 * to the CLI path.
 */
async function tryStudioSnapshot() {
    ...
    try {
        ...
        const screenshotRaw = payload.deviceScreen?.screenshot ?? payload.screenshot;
