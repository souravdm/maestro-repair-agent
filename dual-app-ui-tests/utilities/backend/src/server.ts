...
import { validateAccessibility, formatA11yReport, generateHTMLReport } from './accessibility';
import { runIOSAccessibilityAudit, runAndroidAccessibilityScanner, mergeAccessibilityReports } from './native-a11y-scanner';
import { classifyRecordedSteps, TestStep } from './flowClassifier';
import { generateChatResponse, getQuickHelpTopics, getFeatureDocumentation, ChatContext } from './chatbot';
import { tryStudioSnapshot, hashHierarchy, invalidateStudioAvailability, studioEndpoint } from './studioClient';
import { buildFrameworkIndex, matchSteps, FrameworkIndex } from './frameworkIndex';
...
const FLOW_OUTPUT_DIR = path.join(PROJECT_ROOT, '.maestro', 'flows');
...
let cachedFrameworkIndex: FrameworkIndex | null = null;
let frameworkIndexBuilding = false;

async function getFrameworkIndex(): Promise<FrameworkIndex> {
  if (cachedFrameworkIndex) return cachedFrameworkIndex;
  if (!frameworkIndexBuilding) {
    frameworkIndexBuilding = true;
    buildFrameworkIndex(MAESTRO_ROOT)
...
app.get('/api/docs/recorder', async (_req, res) => {
  const candidates = [
    ...
    path.join(PROJECT_ROOT, 'maestro-recorder', 'README.md'),
    // Repo-level guide, if present.
    path.join(PROJECT_ROOT, 'docs', 'guides', 'MAESTRO_RECORDER.md'),
    // Legacy path some builds referenced.
    path.join(PROJECT_ROOT, 'docs', 'framework-features', 'MAESTRO_FLOW_RECORDER.md'),
  ];
  ...
  res.status(404).type('text/markdown').send(
    '# Documentation not found\n\nExpected one of:\n' +
      candidates.map((c) => `- \`${path.relative(PROJECT_ROOT, c)}\``).join('\n'),
  );
});
...
app.post('/api/framework/match', async (req, res) => {
  try {
    ...
    const matches = matchSteps(steps as Array<{ type: string; target?: string; value?: string }>, idx);
    res.json({
      ready: true,
...
app.post('/api/framework/refresh', async (_req, res) => {
  try {
    ...
    const idx = await buildFrameworkIndex(MAESTRO_ROOT);
...
let hierarchyFetchInFlight: Promise<HierarchyElement[]> | null = null;
let lastHierarchy: HierarchyElement[] = [];
let lastHierarchyAt = 0;
let lastHierarchyHash = '';           // sha1 of last broadcast tree — skip identical resends
let lastStudioScreenshot: string | null = null; // data URL captured with the hierarchy (Studio only)
let lastScreenshotPath: string | null = null; // For color validation
...
let currentDevicePlatform: 'ios' | 'android' = 'ios';
...
interface HierarchyElement {
  ...
  clickable: boolean;
  focused: boolean;
  ocrDetected?: boolean; // true if element was detected via OCR fallback
}

// ── OCR Fallback for Native Elements ──────────────────────────────────────
// Uses Apple Vision framework to detect text in the bottom portion of the
// screen (tab bar area) that the Maestro hierarchy may not expose — e.g.
// native UITabBar labels inside Flutter hybrid apps on iOS 26.x.
...
async function detectScreenTextViaOCR(screenshotPath: string, regionTop?: number, regionBottom?: number): Promise<OCRTextRegion[]> {
  const scriptPath = path.join(__dirname, 'ocr-screen-text.swift');
  try {
    ...
    if (Array.isArray(parsed)) {
      return parsed as OCRTextRegion[];
    }
    return [];
  } catch (error) {
...
async function getScreenScaleFactor(): Promise<number> {
  ...
  try {
    ...
    if (match) {
      ...
      if (pixelWidth > 1000) return 3;
      if (pixelWidth > 700) return 2;
    }
  } catch { /* fallback */ }
...
async function mergeOCRElements(hierarchy: HierarchyElement[], screenshotPath: string | null): Promise<HierarchyElement[]> {
  ...
  try {
    ...
    if (ocrRegions.length === 0) return hierarchy;
    ...
    const existingTexts = new Set(
      hierarchy.map(el => el.text.toLowerCase().trim())
    );
    ...
    for (const region of ocrRegions) {
      ...
      if (existingLongTexts.some(longText => longText.includes(textLower))) continue;
      if (region.text.length < 2) continue;
      if (/^\d+$/.test(region.text)) continue; // Pure numbers (e.g. "00" badge)
      // Skip long text fragments (likely body text, not interactive elements)
      if (region.text.length > 40) continue;
...
const mockHierarchy: HierarchyElement[] = [
...
wss.on('connection', (ws) => {
  ...
  }).catch(() => { /* silent — simulator may not be running */ });
...
function inferElementType(rawType: string, identifier: string, text: string): string {
  ...
  if (
    /textfield|text field|edittext|input|searchfield|secure/.test(t) ||
  ...
  if (
    /^(continue with|continue as|sign in|log in|create account|create an account|join now|get started|shop now|add to cart|checkout|allow|close|done|next|back)/i.test(text)
  ) {
    return 'button';
  }
...
function isSystemElement(text: string, identifier: string): boolean {
...
function parseBoundsRect(bounds: string): { x1: number; y1: number; x2: number; y2: number; width: number; height: number } | null {
...
function isGenericContainer(text: string, identifier: string, rawType: string): boolean {
  const tx = text.toLowerCase();
  const id = identifier.toLowerCase();
  const type = rawType.toLowerCase();

  return (
    /^(view|container|cell|other|group|wrapper|content view|scroll view|table view|collection view|tipview|shopbackground)$/i.test(text) ||
...
function shouldIncludeElement(params: {
  ...
  if (/scroll bar|vertical scroll bar|horizontal scroll bar/i.test(tx)) return false;
  ...
  if (['textField', 'button', 'link', 'switch', 'check'].includes(type)) return true;
  ...
  if (isGenericContainer(tx, id, rawType)) return false;
  
  // Filter out very large elements that are likely backgrounds
  if (rect && rect.width >= 320 && rect.height >= 500 && ['label', 'image', 'unknown'].includes(type)) return false;
  
  // Filter out elements with only generic IDs and no text
  if (!tx && id && isGenericContainer(id, id, rawType)) return false;

  // Include labels and images with text
  return (type === 'label' || type === 'image') && tx.length > 0;
}

function parseBounds(attrs: Record<string, unknown>): string {
...
function getBoundsArea(bounds: string): number {
  const rect = parseBoundsRect(bounds);
  if (!rect) return 0;
  return Math.max(0, rect.width) * Math.max(0, rect.height);
}

function boundsOverlap(a: string, b: string): boolean {
...
function getElementPriority(element: HierarchyElement): number {
...
function collapseDuplicateElements(elements: HierarchyElement[]): HierarchyElement[] {
  const result: HierarchyElement[] = [];
...
function extractHierarchyFromJson(node: unknown, out: HierarchyElement[], depth = 0): void {
...
function parseAccessibilityTreeOutput(rawOutput: string): HierarchyElement[] {
  ...
  if (jsonStart >= 0) {
    try {
      ...
      const deduped = new Map<string, HierarchyElement>();
      for (const el of elements) {
        const key = `${el.type}|${el.text}|${el.bounds}`.toLowerCase();
        if (!deduped.has(key)) deduped.set(key, el);
      }
  ...
  const fallback: HierarchyElement[] = [];
  for (const line of lines) {
    const trimmed = line.trim();
    if (!trimmed || /^Running on/i.test(trimmed) || trimmed === 'None:') continue;
...
async function detectAvailableDevices(): Promise<{ ios: boolean; android: boolean }> {
...
async function getIOSScreenshot(): Promise<string> {
  ...
  try {
    ...
    const screenshotBuffer = await fs.readFile(outputPath);
...
async function getAndroidScreenshot(): Promise<string> {
...
async function getRealSimulatorScreenshot(): Promise<string> {
...
async function getBootedSimulatorUdid(): Promise<string | null> {
  try {
...
async function detectFrontmostApp(): Promise<{ bundleId: string; displayName: string } | null> {
  try {
    const { stdout: launchList } = await execFileAsync(
    ...
    for (const line of launchList.split('\n')) {
      ...
      if (match) runningBundleIds.push(match[1].trim());
    }
...
async function handleDetectApp(ws: any): Promise<void> {
  try {
...
async function getIOSHierarchy(): Promise<HierarchyElement[]> {
  if (process.platform !== 'darwin') {
    return [];
  }
...
async function getAndroidHierarchy(): Promise<HierarchyElement[]> {
  try {
...
function parseTextBasedHierarchy(output: string): HierarchyElement[] {
  const elements: HierarchyElement[] = [];
...
async function getAndroidHierarchyFallback(): Promise<HierarchyElement[]> {
  try {
...
async function getRealHierarchy(): Promise<HierarchyElement[]> {
  const now = Date.now();
  ...
  hierarchyFetchInFlight = (async () => {
    ...
    const studio = await tryStudioSnapshot();
    if (studio) {
    ...
    let hierarchy: HierarchyElement[];
    if (currentDevicePlatform === 'android') {
...
function broadcastHierarchyIfChanged(ws: any, hierarchy: HierarchyElement[]): boolean {
  const hash = hashHierarchy(hierarchy as any);
  if (hash === lastHierarchyHash) return false;
  lastHierarchyHash = hash;
  ws.send(JSON.stringify({ type: 'hierarchy-updated', hierarchy, hash }));
  return true;
}

async function handleStartRecording(ws: any, data: any) {
...
function handleStopRecording(ws: any, data: any) {
...
function handleGetDevices(ws: any) {
...
async function handleTapElement(ws: any, data: any) {
  ...
  try {
    ...
    await fs.unlink(flowPath).catch(() => undefined);
  }
}

async function handleRefreshScreenshot(ws: any) {
...
async function handleGetHierarchy(ws: any) {
...
async function handleTakeScreenshot(ws: any) {
...
async function handleSwitchDevice(ws: any, data: any) {
...
async function handleValidateAccessibility(ws: any) {
  ...
  try {
    ...
    const rawMaestroReport = await validateAccessibility(hierarchy, lastScreenshotPath || undefined);
    ...
    if (lastScreenshotPath) {
      await fs.unlink(lastScreenshotPath).catch(() => undefined);
    ...
    if (platform === 'ios') {
      ...
      try {
        ...
        const iosReport = await runIOSAccessibilityAudit(bundleId);
    ...
    if (platform === 'android') {
      ...
      try {
        ...
        const androidReport = await runAndroidAccessibilityScanner(packageName);
    ...
    // Merge all reports
    const mergedReport = mergeAccessibilityReports(maestroReport, nativeReports);
    
    // Generate HTML report for download
    const htmlReport = generateHTMLReport(mergedReport);
...
function extractPlatformFix(howToFix: string, platform: 'ios' | 'android'): string {
...
function filterA11yFixesForPlatform(report: any, platform: 'ios' | 'android'): any {
...
function detectPlatform(hierarchy: any[]): 'ios' | 'android' | 'both' {
  if (hierarchy.length === 0) return 'both';
  
  // Check element types to determine platform
  const hasIOSTypes = hierarchy.some(el => 
    el.type?.includes('UI') || 
    el.type?.includes('NS') ||
    el.id?.includes('accessibility')
  );
  
  const hasAndroidTypes = hierarchy.some(el =>
    el.type?.includes('android.') ||
    el.id?.includes('resource-id')
  );
  
  if (hasIOSTypes && hasAndroidTypes) return 'both';
  if (hasIOSTypes) return 'ios';
  if (hasAndroidTypes) return 'android';
  
  return 'both'; // Default to checking both
}

async function handleValidatePulse(ws: any) {
  ws.send(JSON.stringify({
...
async function _handleValidatePulse_disabled(ws: any) {
  ...
  try {
    ...
    const hasAndroidElements = hierarchy.some((el: any) => {
    ...
    const hasIOSElements = hierarchy.some((el: any) => {
      const type = (el.type || el.className || '').toLowerCase();
    ...
    function flattenHierarchy(node: any, depth: number): any[] {
      ...
      return [el, ...children.flatMap((c: any) => flattenHierarchy(c, depth + 1))];
    }

    // If hierarchy is already flat (array of elements), use directly; otherwise flatten
    const elements = Array.isArray(hierarchy) && hierarchy.length > 0 && !(hierarchy[0] as any).children
      ? hierarchy
      : hierarchy.flatMap((node: any) => flattenHierarchy(node, 0));

    const violations = (pulseValidator as any)?.validateElements(
      elements,
      { id: 'recorder-live', name: 'Live Screen' },
      platform
    ) ?? [];
    ...
    console.log(`🎨 Color validation: ${lastScreenshotPath ? 'enabled (screenshot available)' : 'disabled (no screenshot)'}`);
    const colorReport = await validateAccessibility(hierarchy, lastScreenshotPath || undefined);
    ...
    for (const v of allViolations) {
      severityCounts[v.severity as keyof typeof severityCounts] =
        (severityCounts[v.severity as keyof typeof severityCounts] || 0) + 1;
...
function generatePulseHTMLReport(report: any): string {
  ...
  const violationRows = report.violations.map((v: any, i: number) => `
  ...
  const componentBreakdownRows = Object.entries(report.componentBreakdown || {})
    ...
    .map(([comp, count]: [string, any]) =>
      `<tr><td style="padding:8px">${comp}</td><td style="padding:8px;text-align:center"><strong>${count}</strong></td></tr>`
    ).join('');
...
function sanitizeFlowName(flowName: string): string {
  return normalizeText(flowName).replace(/[^a-zA-Z0-9-_]+/g, '_') || 'recorded_flow';
}

function inferScreenName(flowName: string, elements: any[]): string {
  const name = normalizeText(flowName);
  
  // Extract screen name from flow name patterns
  if (name.match(/login|signin|sign_in/i)) return 'Login';
  if (name.match(/signup|register|sign_up/i)) return 'Signup';
  if (name.match(/dashboard|home|main/i)) return 'Dashboard';
  if (name.match(/profile|account/i)) return 'Profile';
  if (name.match(/cart|checkout/i)) return 'Cart';
  if (name.match(/search/i)) return 'Search';
  if (name.match(/pharmacy/i)) return 'Pharmacy';
  if (name.match(/benefits|insurance/i)) return 'Benefits';
  if (name.match(/health/i)) return 'Health';
  
  // Analyze element text for context
  const elementTexts = elements.map(e => e.text.toLowerCase()).join(' ');
  if (elementTexts.includes('email') && elementTexts.includes('password')) return 'Login';
  if (elementTexts.includes('sign up') || elementTexts.includes('create account')) return 'Signup';
  if (elementTexts.includes('cart') || elementTexts.includes('checkout')) return 'Cart';
  
  // Default to capitalized flow name
  return name.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join('');
}

function inferCategory(appId: string, elements: any[]): string {
  const id = normalizeText(appId).toLowerCase();
  const elementTexts = elements.map(e => e.text.toLowerCase()).join(' ');
  
  // Check element context first
  if (elementTexts.includes('login') || elementTexts.includes('sign in') || elementTexts.includes('password')) return 'Account';
  if (elementTexts.includes('pharmacy') || elementTexts.includes('prescription') || elementTexts.includes('refill')) return 'Pharmacy';
  if (elementTexts.includes('shop') || elementTexts.includes('cart') || elementTexts.includes('checkout')) return 'Shop';
  if (elementTexts.includes('insurance') || elementTexts.includes('benefits') || elementTexts.includes('claim')) return 'Benefits';
  if (elementTexts.includes('health') || elementTexts.includes('records') || elementTexts.includes('medical')) return 'Health';
  if (elementTexts.includes('home') || elementTexts.includes('dashboard')) return 'Home';
  
  // Check app ID
  if (id.includes('health100')) return 'Health';
  
  return 'Common';
}

function generateElementName(text: string, type: string, index: number): string {
  const normalized = text.replace(/[^a-zA-Z0-9\s]/g, '').trim();
  const words = normalized.split(/\s+/).filter(w => w.length > 0);
  
  // Generate camelCase name
  let baseName = words.map((w, i) => 
    i === 0 ? w.toLowerCase() : w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()
  ).join('');
  
  // Add type suffix based on element type
  if (type === 'input') {
    if (!baseName.endsWith('Field')) baseName += 'Field';
  } else if (type === 'tap') {
    if (text.toLowerCase().includes('button') || text.toLowerCase().includes('btn')) {
      if (!baseName.endsWith('Btn')) baseName += 'Btn';
    } else if (text.toLowerCase().includes('link')) {
      if (!baseName.endsWith('Link')) baseName += 'Link';
    ...
    if (text.toLowerCase().includes('error') || text.toLowerCase().includes('message')) {
      if (!baseName.endsWith('Msg')) baseName += 'Msg';
    } else if (text.toLowerCase().includes('title') || text.toLowerCase().includes('heading')) {
      if (!baseName.endsWith('Title')) baseName += 'Title';
    } else {
      if (!baseName.endsWith('Label')) baseName += 'Label';
    }
  }
...
function generateScreenObject(elements: any[], screenName: string, category: string): string {
...
function isSyntheticHierarchyId(value: string): boolean {
  const normalized = normalizeText(value);
  return /^element-\d+-\d+$/i.test(normalized) || /^id:\s*element-\d+-\d+$/i.test(normalized);
}

function stepToYaml(step: any): string {
...
function getStepTarget(step: any): string {
  return normalizeText(step?.selectorId || step?.target);
}

function hierarchyContainsTarget(hierarchy: HierarchyElement[], target: string): boolean {
...
function findBestMatchingHierarchyElement(hierarchy: HierarchyElement[], step: any): HierarchyElement | null {
...
async function enrichStepWithResolvedSelector(step: any): Promise<any> {
  if (!step) {
    return step;
  }
...
function buildPointTapYaml(bounds: string): string | null {
  const rect = parseBoundsRect(bounds);
  if (!rect) return null;
...
function buildExecutionStepYaml(step: any): string {
...
async function executeNonInteractiveStep(step: any): Promise<{ handled: boolean; message?: string }> {
  if (step?.type === 'wait') {
...
async function runMaestroCommand(args: string[], timeoutMs: number): Promise<{ stdout: string; stderr: string }> {
  return new Promise((resolve, reject) => {
    const child = execFile('maestro', args, {
      maxBuffer: 20 * 1024 * 1024
    }, (error, stdout, stderr) => {
      if (timeoutHandle) {
...
async function handleExportFlow(ws: any, data: any) {
  ...
  const outputPath = path.join(FLOW_OUTPUT_DIR, `${safeFlowName}.yaml`);

  await fs.mkdir(FLOW_OUTPUT_DIR, { recursive: true });
...
async function handleExecuteStep(ws: any, data: any) {
  ...
  try {
    ...
    setTimeout(() => {
      fs.unlink(flowPath).catch(() => undefined);
    }, 2000);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
...
async function handleExecuteAllSteps(ws: any, data: any) {
...
function handlePlayback(ws: any, data: any) {
...
async function handleClassifySteps(ws: any, data: any) {
  ...
  try {
    ...
    const classification = classifyRecordedSteps(steps);
...
async function handleChatMessage(ws: any, data: any) {
  ...
  try {
    ...
    const response = generateChatResponse(data.message, context);
...
// Help Topics Handler
function handleGetHelpTopics(ws: any) {
  console.log('📚 Sending help topics...');
  
  try {
    const topics = getQuickHelpTopics();
...
server.listen(PORT, async () => {
  ...
  const studio = await tryStudioSnapshot();
  if (studio) {
    console.log(`⚡ Hierarchy source: Maestro Studio (${studioEndpoint()}) — fast path`);
  } else {
