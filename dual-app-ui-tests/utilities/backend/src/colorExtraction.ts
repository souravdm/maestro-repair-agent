...
import { CVS_COLOR_TOKENS, DEPRECATED_COLOR_FAMILIES, COLOR_USAGE_GUIDELINES, ColorToken } from './cvsColorTokens';
...
interface ColorMatch {
  token: ColorToken;
  deltaE: number; // Color difference (0 = exact, >10 = noticeably different)
 ...
 */
export async function extractColorFromRegion(
...
export function findClosestColorToken(
  ...
  let closestMatch: ColorMatch = {
    token: CVS_COLOR_TOKENS[0],
    deltaE: Infinity,
    confidence: 'different'
  };
  
  for (const token of CVS_COLOR_TOKENS) {
    const deltaE = diff(
      { R: extractedColor.rgb.r, G: extractedColor.rgb.g, B: extractedColor.rgb.b },
      { R: token.rgb.r, G: token.rgb.g, B: token.rgb.b }
    );
...
export function validateColor(
  ...
  if (DEPRECATED_COLOR_FAMILIES.includes(match.token.family)) {
    violations.push({
      severity: 'error',
 ...
 */
function validateSemanticColorUsage(
  token: ColorToken,
  elementType: string,
  elementText: string,
  elementId: string
): Omit<ColorViolation, 'extractedColor' | 'expectedToken' | 'deltaE'> | null {
  ...
  if (family === 'red' && !isErrorElement(text, id, type)) {
    return {
      ...
      suggestion: `Red is reserved for error states. Use Rose for brand color or another appropriate family. See: ${COLOR_USAGE_GUIDELINES.red}`
    };
  }
  ...
  if (family === 'teal' && !isSuccessElement(text, id, type)) {
    return {
      ...
      suggestion: `Teal is typically used for success states. Verify this is intentional. See: ${COLOR_USAGE_GUIDELINES.teal}`
    };
  }
  ...
  if (family === 'tangerine' && !isWarningElement(text, id, type)) {
    return {
      ...
      suggestion: `Tangerine is typically used for warning states. Verify this is intentional. See: ${COLOR_USAGE_GUIDELINES.tangerine}`
    };
  }
...
export async function validateTextContrast(
  ...
  try {
    ...
    if (!textColor) return violations;
    ...
    const contrastRatio = ccc.getContrastRatio(
      ...
      [bgColor.rgb.r, bgColor.rgb.g, bgColor.rgb.b]
    );
...
function getConfidenceLevel(deltaE: number): 'exact' | 'close' | 'similar' | 'different' {
...
function isErrorElement(text: string, id: string, type: string): boolean {
  const errorKeywords = ['error', 'invalid', 'failed', 'wrong', 'incorrect', 'danger', 'alert', 'critical'];
  return errorKeywords.some(keyword => 
    text.includes(keyword) || id.includes(keyword) || type.includes(keyword)
  );
}

function isSuccessElement(text: string, id: string, type: string): boolean {
...
function isWarningElement(text: string, id: string, type: string): boolean {
