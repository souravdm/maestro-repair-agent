...
export interface FrameworkEntry {
  relativePath: string;      // relative from .maestro/, e.g. "subflows/Account/authentication/login.yaml"
  type: 'flow' | 'subflow';
  name: string;              // file basename without .yaml
  domain: string;            // first-level directory segment, e.g. "Account", "Benefits"
  tags: string[];
  hasLaunchApp: boolean;
  textTokens: string[];      // sorted, deduplicated normalised text targets
  actionTypes: string[];     // ordered action-type list
  runFlowRefs: string[];     // subflow basenames referenced via runFlow
  stepCount: number;
...
export interface FrameworkIndex {
  ...
  screens: string[];         // relative paths, e.g. "screens/Account/LoginScreen.js"
  totalFiles: number;
  buildTimeMs: number;
}
...
function tokenFromStep(step: unknown): StepToken | null {
  ...
  switch (key) {
    ...
    case 'runFlow': {
      const ref = typeof val === 'string' ? val
        : (val && typeof val === 'object' ? String((val as Record<string, unknown>)['file'] ?? '') : '');
      return { actionType: 'runflow', text: path.basename(ref, '.yaml') };
    }
...
function walkSteps(steps: unknown[], out: StepToken[]): void {
  ...
  for (const step of steps) {
    ...
    if (step && typeof step === 'object') {
      ...
      const nested = (s['commands']
        ?? (s['runFlow'] as Record<string, unknown> | undefined)?.['commands']
        ?? (s['when'] as Record<string, unknown> | undefined)?.['commands']
      );
      if (Array.isArray(nested)) walkSteps(nested, out);
    }
  }
}
...
async function parseFile(
  ...
  try {
    ...
    if (parts.length >= 2) {
      try { frontmatter = (parseYaml(parts[0]) as Record<string, unknown>) ?? {}; } catch { /* ignore */ }
      bodyText = parts.slice(1).join('\n---\n');
    }

    let steps: unknown = null;
    try { steps = parseYaml(bodyText); } catch { return null; }
    if (!Array.isArray(steps)) return null;

    const tokens: StepToken[] = [];
    walkSteps(steps, tokens);

    const rel = path.relative(maestroRoot, filePath);
...
async function walkDir(dir: string, ext: string): Promise<string[]> {
  ...
  try {
    ...
    await Promise.all(entries.map(async (e) => {
      ...
      if (e.isDirectory()) out.push(...await walkDir(full, ext));
...
export async function buildFrameworkIndex(maestroRoot: string): Promise<FrameworkIndex> {
  const t0 = Date.now();
  const [subflowFiles, flowFiles, screenFiles] = await Promise.all([
    walkDir(path.join(maestroRoot, 'subflows'), '.yaml'),
    walkDir(path.join(maestroRoot, 'flows'),    '.yaml'),
    walkDir(path.join(maestroRoot, 'screens'),  '.js'),
  ]);
  ...
  const subflows = rawSubflows.filter((e): e is FrameworkEntry => e !== null);
  const flows    = rawFlows.filter((e): e is FrameworkEntry => e !== null);
  const screens  = screenFiles.map(f => path.relative(maestroRoot, f));
...
export function matchSteps(
  recordedSteps: Array<{ type: string; target?: string; value?: string }>,
  ...
  const tokens: StepToken[] = recordedSteps.map(step => {
  ...
  for (const entry of index.subflows) {
    ...
    if (score > 0.08) {
      ...
      const parts: string[] = [];
      if (textScore > 0.15)   parts.push(`${pct(textScore)} element text overlap`);
      if (actionScore > 0.35) parts.push(`${pct(actionScore)} action sequence match`);
