...
exports.A11ySeverity = exports.A11yCategory = void 0;
exports.validateAccessibility = validateAccessibility;
exports.formatA11yReport = formatA11yReport;
exports.generateHTMLReport = generateHTMLReport;
const colorExtraction_1 = require("./colorExtraction");
// WCAG 2.1 Accessibility Categories
var A11yCategory;
(function (A11yCategory) {
    A11yCategory["PERCEIVABLE"] = "Perceivable";
    A11yCategory["OPERABLE"] = "Operable";
    A11yCategory["UNDERSTANDABLE"] = "Understandable";
    A11yCategory["ROBUST"] = "Robust";
})(A11yCategory || (exports.A11yCategory = A11yCategory = {}));
var A11ySeverity;
(function (A11ySeverity) {
    A11ySeverity["CRITICAL"] = "critical";
    A11ySeverity["SERIOUS"] = "serious";
    A11ySeverity["MODERATE"] = "moderate";
    A11ySeverity["MINOR"] = "minor";
})(A11ySeverity || (exports.A11ySeverity = A11ySeverity = {}));
...
//
async function validateAccessibility(hierarchy, screenshotPath) {
    const violations = [];
    let violationId = 1;
    ...
    for (const element of hierarchy) {
        ...
        if (element.clickable && bounds) {
            ...
            for (const other of elementBounds) {
                if (other.element === element || !other.element.clickable || !other.bounds)
                    continue;
        ...
        if ((element.type === 'view' || element.type === 'group') && !element.clickable) {
            ...
            if (containerBounds) {
                let childCount = 0;
                for (const other of elementBounds) {
                    if (other.element === element || !other.bounds)
                        continue;
    ...
    if (screenshotPath) {
        ...
        for (const element of hierarchy) {
            ...
            try {
                ...
                const extractedColor = await (0, colorExtraction_1.extractColorFromRegion)(screenshotPath, bounds);
                if (!extractedColor)
                    continue;
                colorsAnalyzed++;
                // Validate color against CVS Pulse tokens
                const colorViolations = (0, colorExtraction_1.validateColor)(extractedColor, element.type, element.text, element.id);
                // Add color violations to main violations list
                for (const cv of colorViolations) {
                    const match = (0, colorExtraction_1.findClosestColorToken)(extractedColor);
                    violations.push({
                    ...
                    // Track statistics
                    if (cv.message.includes('deprecated'))
                        deprecatedColors++;
                    if (cv.deltaE && cv.deltaE > 5)
                        tokenMismatches++;
                }
                // Text contrast validation for text elements
                if (element.type === 'label' || element.type === 'button' || element.type === 'textField') {
                    const contrastViolations = await (0, colorExtraction_1.validateTextContrast)(screenshotPath, bounds, 14);
...
function formatA11yReport(report) {
