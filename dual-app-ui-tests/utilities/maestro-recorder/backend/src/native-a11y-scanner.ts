 ...
 */
export async function runIOSAccessibilityAudit(
  bundleId: string,
  simulatorId?: string
): Promise<NativeA11yReport> {
  const issues: NativeA11yIssue[] = [];
  
  try {
    // Get booted simulator if not specified
    if (!simulatorId) {
      const { stdout } = await execAsync(
        'xcrun simctl list devices | grep "(Booted)" | grep -o "[A-F0-9]\\{8\\}-[A-F0-9]\\{4\\}-[A-F0-9]\\{4\\}-[A-F0-9]\\{4\\}-[A-F0-9]\\{12\\}" | head -1'
      );
    ...
    const { stdout, stderr } = await execAsync(
      `xcrun simctl spawn ${simulatorId} log stream --predicate 'subsystem == "com.apple.accessibility"' --level debug --timeout 5`,
      { timeout: 10000 }
    ).catch(() => ({ stdout: '', stderr: '' }));
 ...
 */
async function runIOSAccessibilityInspector(
  simulatorId: string,
  bundleId: string
): Promise<NativeA11yIssue[]> {
  const issues: NativeA11yIssue[] = [];

  try {
    // Use simctl to get accessibility hierarchy
    const { stdout } = await execAsync(
      `xcrun simctl spawn ${simulatorId} log show --predicate 'subsystem == "com.apple.accessibility" AND category == "AXElement"' --last 1m --style compact`
    ).catch(() => ({ stdout: '' }));
 ...
 */
export async function runAndroidAccessibilityScanner(
  packageName: string,
  deviceId?: string
): Promise<NativeA11yReport> {
  const issues: NativeA11yIssue[] = [];
  ...
  try {
    ...
    const { stdout: hierarchyXml } = await execAsync(
 ...
 */
async function runAndroidAccessibilityScannerAPI(
  deviceId: string,
  packageName: string
): Promise<NativeA11yIssue[]> {
  const issues: NativeA11yIssue[] = [];
  ...
  try {
    ...
    const { stdout } = await execAsync(
      `adb -s ${deviceId} shell pm list packages | grep "com.google.android.apps.accessibility.auditor"`
    );
    ...
    await execAsync(
      `adb -s ${deviceId} shell am start -n com.google.android.apps.accessibility.auditor/.MainActivity`
    );
    ...
    const { stdout: logcat } = await execAsync(
      `adb -s ${deviceId} logcat -d -s AccessibilityScanner:* | tail -100`
    );
...
function parseIOSAuditLine(line: string): NativeA11yIssue | null {
...
function parseIOSAccessibilityHierarchy(logOutput: string): IOSAccessibilityElement[] {
  ...
  for (const line of lines) {
    if (line.includes('AXElement')) {
      ...
      const element: IOSAccessibilityElement = {
...
interface AndroidAccessibilityElement {
  resourceId?: string;
...
function parseAndroidAccessibilityHierarchy(xml: string): AndroidAccessibilityElement[] {
  // Parse Android UI hierarchy XML
  // This is a simplified parser - actual implementation would use a proper XML parser
  const elements: AndroidAccessibilityElement[] = [];
  ...
  if (matches) {
    for (const match of matches) {
      ...
      if (resourceIdMatch) element.resourceId = resourceIdMatch[1];

      const contentDescMatch = match.match(/content-desc="([^"]+)"/);
      if (contentDescMatch) element.contentDescription = contentDescMatch[1];
      ...
      if (boundsMatch) element.bounds = boundsMatch[1];
...
function parseAndroidScannerResults(logcat: string): NativeA11yIssue[] {
...
function parseFrame(frameStr: string): { x: number; y: number; width: number; height: number } | null {
...
export function mergeAccessibilityReports(
  maestroReport: any,
  nativeReports: NativeA11yReport[]
): any {
